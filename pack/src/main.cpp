#include <cstdlib>
#include <filesystem>
#include <stdexcept>
#include <iostream>

#include "build_info.hpp"
#include "core/demangle.hpp"
#include "core/gitignore.hpp"
#include "pack_file.hpp"

int main(int argc, char *argv[]) {
	try {
		if (argc < 1) throw std::runtime_error("Did not recieve application path in argv.");

		#ifdef DEBUG
		prog_info();
		#endif

		core::sfs_settings settings = core::load_settings("sfs_settings.txt");

		std::vector<std::filesystem::path> paths = core::gitignore_get_paths(settings);
		for (std::filesystem::path path : paths) {
			std::uint32_t count = pack_file(settings, path);
			if (count > 0) std::cout << "Packed " << count << " chunks into '" << path.string() << "'.\n";
		}

		std::cout << "Done.\n";
		std::cin.get();
		return EXIT_SUCCESS;
	} catch (std::exception const& e) {
		std::cerr << "'" << core::demangle(typeid(e).name()) << "': " << e.what() << '\n';
		std::cin.get();
		return EXIT_FAILURE;
	}
}
